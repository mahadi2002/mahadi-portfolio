      ...
      <div className="mx-auto max-w-5xl px-6">
        ...
        <div className="mt-12 grid grid-cols-1 gap-x-12 gap-y-10 md:grid-cols-2">
          {skills.map((group, i) => (
            ...
            >
              <p className="font-mono text-xs uppercase tracking-[0.14em] text-zinc-600">{group.label}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {group.items.map((item) => (
                  <span
                    key={item}
                    className="rounded-full border border-white/10 bg-white/[0.02] px-3.5 py-1.5 text-sm text-zinc-300 transition-colors hover:border-amber-400/40 hover:text-amber-200"
                  >
                    {item}
                  </span>
                ))}
